...
use Infobank\Send\Omni\Destinations;
...
class OmniTest extends TestCase
{
    ...
    public function check_assert(
        ...
        foreach ($response->getData()->getDestinations() as $destination){
            $this->assertEquals(
                $destination->getCode(),
                "A000",
                "Failed.\nresponse:" . json_encode($destination, true) . "\nrequest:" . json_encode($message)
            );
            $this->assertEquals(
    ...
    {
        $message = (new OmniMessage(array(
            new Destinations("01012341234"),
            new Destinations("01012341235")
        )))
            ->addMessageFlow(
